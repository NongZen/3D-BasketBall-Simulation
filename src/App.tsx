...
export default function App() {
  ...
  useEffect(() => {
    ...
    // --- ป้องกัน React Strict Mode สร้าง Canvas ซ้ำซ้อน ---
    ...
    renderer.setPixelRatio(window.devicePixelRatio); // ทำให้ภาพคมชัดบนจอมือถือ/แท็บเล็ต
    ...
    const animate = (time: number) => {
      ...
      if (stateRef.current.isShooting) {
        ...
        const dx = target.x - startX;
        const dz = target.z - startZ;
        const dirX = dx / Math.sqrt(dx * dx + dz * dz);
        const dirZ = dz / Math.sqrt(dx * dx + dz * dz);
  ...
  return (
    // บังคับ Layout ด้วย Inline Style เพื่อแก้ปัญหา Tailwind h-screen บั๊ก
    <div style={{ position: 'relative', width: '100vw', height: '100vh', backgroundColor: '#111827', overflow: 'hidden' }} className="font-sans">
      ...
      {/* UI Overlay */}
      <div className="ui-overlay absolute top-4 left-4 bg-gray-900/85 backdrop-blur-md p-5 rounded-2xl shadow-2xl border border-gray-700 w-80 text-white select-none z-10 max-h-[95vh] overflow-y-auto">
        <h1 className="text-xl font-bold mb-4 text-blue-400">3D Hoops Sim</h1>
        
        <div className="flex bg-gray-800 p-1 rounded-lg mb-4 border border-gray-700">
          <button
            onClick={() => setInteractionMode('camera')}
            className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-colors ${interactionMode === 'camera' ? 'bg-blue-600 text-white shadow' : 'text-gray-400'}`}
          >
            🎥 หมุนกล้อง
          </button>
          <button
            onClick={() => setInteractionMode('shooter')}
            className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-colors ${interactionMode === 'shooter' ? 'bg-blue-600 text-white shadow' : 'text-gray-400'}`}
          >
            🏃 ย้ายจุดยิง
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div className="space-y-2 bg-gray-800/50 p-3 rounded-lg border border-gray-700">
            <div>
              <div className="flex justify-between mb-1"><label className="text-xs text-gray-400">ตำแหน่ง X</label><span className="text-xs text-blue-300">{startX}m</span></div>
              <input type="range" min="-7.5" max="7.5" step="0.1" value={startX} onChange={(e) => { setStartX(parseFloat(e.target.value)); setIsShooting(false); }} className="w-full accent-blue-500 h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
            </div>
            <div>
              <div className="flex justify-between mb-1"><label className="text-xs text-gray-400">ตำแหน่ง Z</label><span className="text-xs text-blue-300">{startZ}m</span></div>
              <input type="range" min="-14" max="0" step="0.1" value={startZ} onChange={(e) => { setStartZ(parseFloat(e.target.value)); setIsShooting(false); }} className="w-full accent-blue-500 h-1.5 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
            </div>
          </div>

          <div>
            <div className="flex justify-between mb-1"><label className="text-sm text-gray-300">องศายิง (Angle)</label><span className="text-sm text-emerald-400">{angle}°</span></div>
            <input type="range" min="10" max="85" step="1" value={angle} onChange={(e) => { setAngle(parseFloat(e.target.value)); setIsShooting(false); }} className="w-full accent-emerald-500 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
          </div>

          <div>
            <div className="flex justify-between mb-1"><label className="text-sm text-gray-300">แรงยิง (Force)</label><span className="text-sm text-emerald-400">{force} N</span></div>
            <input type="range" min="10" max="100" step="0.1" value={force} onChange={(e) => { setForce(parseFloat(e.target.value)); setIsShooting(false); }} className="w-full accent-emerald-500 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <label htmlFor="lineStyle" className="text-sm text-gray-300 cursor-pointer">เส้นวิถีแบบประ (Dashed)</label>
          </div>
          ...
          <div className="space-y-2">
            <button onClick={() => calculateAutoAim(HOOP_CENTER, 'swish')} className="w-full py-2 px-4 bg-gray-800 hover:bg-gray-700 border border-gray-600 rounded-lg text-sm transition-colors flex justify-between items-center">
              <span>🎯 Auto-Aim (ลงห่วง)</span>{aimMode === 'swish' && <span className="w-2 h-2 rounded-full bg-emerald-500"></span>}
            </button>
            <button onClick={() => calculateAutoAim(VIRTUAL_HOOP_CENTER, 'bank')} className="w-full py-2 px-4 bg-gray-800 hover:bg-gray-700 border border-gray-600 rounded-lg text-sm transition-colors flex justify-between items-center">
              <span>📐 Auto-Aim (กระทบแป้น)</span>{aimMode === 'bank' && <span className="w-2 h-2 rounded-full bg-emerald-500"></span>}
            </button>
